...
class ssServerDatabase(Database):
    ...
    def getItem(self,service_idf):
        try:
            c = self.cursor
            _str = "SELECT service_idf,server_port,password,method,timeout FROM ss_server WHERE `service_idf` = ?"
    ...
    def deleteInstance(self,service_idf):
        try:
            c = self.cursor
            delete_str = "DELETE FROM ss_server WHERE service_idf = ?"
    ...
    def portCollision(self,dest_port):
        try:
            c = self.cursor
            port_str = "SELECT service_idf FROM ss_server WHERE server_port = ?"
